...
Повторное выполнение playbook с флагом `--diff` подтверждает, что все изменения уже были применены ранее, и Ansible не вносит избыточных правок.  
Это гарантирует идемпотентность playbook.

**Повторный запуск Playbook с отображением изменений**
...
- **ClickHouse** – столбцовая аналитическая СУБД. Устанавливается из официального deb-репозитория, сервер запускается вручную (без systemd).
- **Vector** – сборщик и трансформатор логов. Устанавливается из бинарного архива, конфигурация генерируется через Jinja2-шаблон.
- **LightHouse** – веб-интерфейс для ClickHouse. Устанавливается Nginx, в него разворачивается статика из репозитория VKCOM (ветка `master`), конфигурация Nginx создаётся из шаблона.

Все операции выполняются с повышением привилегий (`become: true`).  
При изменении конфигурации Vector или Nginx соответствующие обработчики автоматически перезапускают сервисы.

## Параметры

| Переменная      | Значение по умолчанию | Описание                                 |
|-----------------|------------------------|------------------------------------------|
| `vector_version`| `"0.42.0"`             | Версия Vector, загружаемая из релизов.  |

Переменные можно переопределить в инвентаре, в файлах `group_vars` или через `--extra-vars`.

## Теги

По умолчанию задачи не содержат тегов. При необходимости их можно добавить в конкретные задачи, например:
...
# Задание 10 – Готовый playbook выложить в свой репозиторий и поставить тег
