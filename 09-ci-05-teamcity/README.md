...
## Задание 17 – Повторная сборка master и проверка артефактов

После устранения ошибки деплоя (увеличена версия до `0.0.3` в `pom.xml`) и настройки сохранения артефактов (`Artifact paths`), была запущена повторная сборка ветки `master`.  
Сборка прошла успешно, все тесты пройдены, артефакт `plaindoll-0.0.3.jar` сохранён в сборке и опубликован в Nexus.

### Выполненные шаги

- В TeamCity запущена сборка конфигурации `Build` для ветки `master`.
- Статус сборки: `Success`.
- Количество пройденных тестов: 6.
- На вкладке **Artifacts** отображается файл `plaindoll-0.0.3.jar`.
- В Nexus (репозиторий `maven-releases`) доступна новая версия артефакта.

### Результат

Сборка master стабильно проходит, артефакты корректно сохраняются и публикуются.  
Все компоненты CI/CD работают без ошибок.
